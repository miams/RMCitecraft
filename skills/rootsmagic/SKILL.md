...
Use the census matching algorithm from `census_rmtree_matcher.py` for automated matching.
...
- `references/schema-reference.md` - Condensed table schemas
- `references/query-cookbook.md` - Common SQL patterns
- `references/fact-types.md` - Event type codes
- `references/date-encoding.md` - Date format specification

## Utility Scripts

| Script | Purpose |
|--------|---------|
| `scripts/inspect-db.py` | Explore database structure and data |

## Common Tasks

### Find all census citations for a person
```python
# See query-cookbook.md for complete implementation
# Must check both EventTable (owned) and WitnessTable (shared)
```

### Update citation formatting
```python
# Always get user approval first
# Read current values before modifying
# Use connect_rmtree() with read_only=False
```

### Export data for analysis
```python
# Use pandas for CSV/Excel export
# Preserve RootsMagic data types
# Document export date and criteria
```
